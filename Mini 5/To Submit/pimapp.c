...
void deleteNode(struct PersonalInfoRecord ** head, char idDel[10], int * counterche) {
  // Store head node
  struct PersonalInfoRecord * temp = * head, * prev;

  // If head node is the one that needs to be deleted
  //if last idDel is not the same as current idDel, execute:
...
void printList(struct PersonalInfoRecord * curr, int numNodes) {
  if (numNodes == 0) {
    putc('\n', stdout);
  } else {
    while (curr != NULL) {
...
int main(int argc, char * argv[]) {
  ...
  if (argc != 2) {
    printf("Error, please pass a database filename as the only argument.\n");
    printf("Usage ./pimapp <dbfile>\n");
    return 1;
  }

  //create head node and allocate space for it; assign temp node to traverse linked list
  struct PersonalInfoRecord * theHead = (struct PersonalInfoRecord * ) malloc(sizeof(struct PersonalInfoRecord)); // = (struct PersonalInfoRecord * ) malloc(sizeof(struct PersonalInfoRecord));
  ...
  while (fgets(inputbuffer, 100, stdin) != NULL) // Get each input line.
  {
    ...
    //If we need to delete a node, aka input starts with D
      ...
      if (nodeCounter != 0) { //don't allow deletions after the linked list has size 0
  ...
  FILE * output;
  if (nodeCounter == 0) { //if no nodes, write nothing
    ...
    while (curr != NULL) { //iterates through linked list
      fprintf(output, "%s,%c,%s,", curr -> id, curr -> ptype, curr -> name); //writes lines to ouput
      //if it's a professor node, use these field replacements
      if (curr -> ptype == 'P') {
      ...
      //go to next PersonalInfoRecord in linked list
      struct PersonalInfoRecord * byebye = curr;
      curr = curr -> next;
      free(byebye); //free memory from last node each time we go to another one, so they're all free by the end
    }
